...
def main(context):
    ...
    # ID del database
    database_id = "6729f7e30033d3541307"

    try:
        if context.req.path == "/":
            # Recupera tutti i documenti dalla collection
            body = context.req.json if context.req.json else {}
            collection_id = body.get("collection_id")
            if not collection_id:
                return context.res.json(
                    {"error": "collection_id è obbligatorio."},
                    status=400,
                )

            documents = databases.list_documents(database_id=database_id, collection_id=collection_id)
            context.log("Documenti recuperati: " + str(documents))
            ...
            if not collection_id:
                return context.res.json(
                    {"error": "collection_id non trovato nell'URL."},
                    status=400,
                )
            ...
            # Aggiorna il documento nella collection
            ...
            context.log("Documento aggiornato: " + str(response))
